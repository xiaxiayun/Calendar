...
public class CalendarView extends LinearLayout {
    ...
    protected void onFinishInflate() {
        ...
        mListViewWeeks.setHasFixedSize(true);   //如果可以确定每个item的高度是固定的，设置这个选项可以提高性能
        mListViewWeeks.setItemAnimator(null);   //设置recyclerView的子动画
        mListViewWeeks.setSnapEnabled(true);  //设置是否能折叠recyclerView
        ...
        BusProvider.getInstance().toObserverable()
                .subscribe(event -> {
                    if (event instanceof Events.CalendarScrolledEvent) {// CalendarView 的滑动事件
                        expandCalendarView();       //扩展CalendarView，设置为其显示5行的高度
                    } else if (event instanceof Events.AgendaListViewTouchedEvent) {//日程list的触摸事件，只要一触摸就变为两行的高度
                        collapseCalendarView(); //折叠CalendarView，设置为其只显示两行的高度
                    ...
                    } else if (event instanceof Events.SignedEvent) {   //
                        Events.SignedEvent signedEvent = (Events.SignedEvent) event;
                        Calendar c = Calendar.getInstance();
                        c.setTime(signedEvent.dayItem.getDate());
                        updateSelectedDay(c, signedEvent.dayItem);  //签到更新日期背景
                    }
                });
    }
