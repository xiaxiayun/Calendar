 ...
 * 行程视图的StickyHeaderListView的头视图
 * StickyHeaderListView 高度可动态配置CalendarView和AgendaHeaderView（使用GridView实现）
 * StickyHeaderListView 主要是通过 ListView 添加头部实现  将复杂的头部分解为 CalendarView和AgendaHeaderView
...
public class AgendaHeaderView extends LinearLayout {
    ...
    // 继承的构造方法
    /**
     * 获取xml文件中属性值用的接口 优先级Context > AttributeSet > defStyleAttr > defStyleRes
     * 在运行时（构造函数）获取属性值
     * 将获取到的属性值应用到自定义View
     */
